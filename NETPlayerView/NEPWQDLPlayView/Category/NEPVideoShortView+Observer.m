...
@implementation NEPVideoShortView (Observer)
...
- (void)creatNSNotifion
{
    ...
    // 进入前台
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationInForeGround) name:UIApplicationDidBecomeActiveNotification object:self];
    // 进入后台
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationInBackGround) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)NELivePlayerPlayBackFinished:(NSNotification*)notification
{
    switch ([[[notification userInfo] valueForKey:NELivePlayerPlaybackDidFinishReasonUserInfoKey] intValue])
    {
        case NELPMovieFinishReasonPlaybackEnded:
        {
           ...
           if ([self.delegate respondsToSelector:@selector(changeplayerURLWithPlayerFinish)]) {
                ...
                if ([self.delegate respondsToSelector:@selector(updataTime:)]) {
                    [self.delegate updataTime:self.curTime];
                }
            }
        ...
        case NELPMovieFinishReasonPlaybackError:
        {
            ...
            [self.bufferingIndicate startAnimating];
            self.agaTimer = [NSTimer scheduledTimerWithTimeInterval:30 target:self selector:@selector(agaTimeAction) userInfo:nil repeats:NO];
            if ([self.delegate respondsToSelector:@selector(setShortTime:)]) {
                [self.delegate setShortTime:self.curTime];
            }
        }
